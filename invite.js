...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`<a:rawda_ates:776538893805420554>${member} Adlı Kişi Sunucuya Katıldı **Davet Eden:** ${invite.inviter.tag} Toplam Davetleri: / **${total + bonus}**  <a:yeiltik:732990476902334495>`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`<a:rawda_ates:776538893805420554> ${member.user.tag} Sunucudan Ayrıldı **Davet Eden:** ${user.tag} Toplam Davetleri: / **${Number(total) + Number(bonus)}** <a:krmzok:732990646830366811>`)
     }
});
